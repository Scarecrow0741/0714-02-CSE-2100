...
void draw_button(SDL_Renderer* renderer, SDL_Rect* rect, SDL_bool is_selected, SDL_bool is_hovered) {
    ...
    SDL_SetRenderDrawColor(renderer, 
        is_selected ? 60 : (is_hovered ? 50 : 40),
        is_selected ? 60 : (is_hovered ? 50 : 40),
        is_selected ? 80 : (is_hovered ? 70 : 60),
        255);
    SDL_RenderFillRect(renderer, rect);

    // Draw button border
    SDL_SetRenderDrawColor(renderer, 
        is_selected ? 100 : (is_hovered ? 80 : 60),
        is_selected ? 100 : (is_hovered ? 80 : 60),
        is_selected ? 140 : (is_hovered ? 120 : 100),
        255);
...
void draw_menu(SDL_Renderer* renderer, TTF_Font* font, MenuOption selected) {
    ...
    if (play_surface) {
        ...
        g_play_button_rect.x = window_w/2 - (play_surface->w / 2) - 40;
        g_play_button_rect.y = window_h * 0.5 - 15;
        ...
        draw_button(renderer, &g_play_button_rect, selected == MENU_PLAY, isHovered);
    ...
    if (exit_surface) {
        ...
        g_exit_button_rect.y = window_h * 0.65 - 15;  // Increased spacing
        ...
        draw_button(renderer, &g_exit_button_rect, selected == MENU_EXIT, isHovered);
